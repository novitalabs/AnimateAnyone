...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def enable_sequential_cpu_offload(self, gpu_id=0):
        ...
        for cpu_offloaded_model in [self.reference_unet, self.denoising_unet]:
    ...
    def decode_latents(self, latents, decoder_consistency=None):
        ...
        video = []
        for frame_idx in tqdm(range(latents.shape[0]), disable=latents.shape[0] < 2):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        batch_size = len(ref_image) if isinstance(ref_image, list) else 1

        #resized_ref_image = [img.resize((224, 224)) for img in ref_image] if isinstance(ref_image, list) else ref_image.resize((224, 224))
        resized_ref_image = [self.square_pad(img) for img in ref_image] if isinstance(ref_image, list) else self.square_pad(ref_image)

        # Prepare clip image embeds
        clip_image = self.clip_image_processor.preprocess(
            resized_ref_image, return_tensors="pt"
        ).pixel_values
        ...
        # pose_cond_tensor = torch.zeros_like(pose_cond_tensor)
        ...
        # Post-processing
        image = self.decode_latents(latents, decoder_consistency=decoder_consistency)  # (b, c, 1, h, w)

        # Convert to tensor
        if output_type == "tensor":
            image = torch.from_numpy(image)
        elif output_type == "pil":
            image = self.numpy_to_pil(np.transpose(image.squeeze(2), (0, 2, 3, 1)))
    ...
    def prepare_unet_args (
        ...
        **kwargs,
        ...
        return dict(
            ...
            do_classifier_free_guidance=do_classifier_free_guidance,
            timesteps=timesteps,
            resized_ref_image=resized_ref_image,
    ...
    def generate_progressive (self,
        ...
        image = self.decode_latents(latents, decoder_consistency=decoder_consistency)  # (b, c, 1, h, w)
        yield self.numpy_to_pil(np.transpose(image.squeeze(2), (0, 2, 3, 1)))
